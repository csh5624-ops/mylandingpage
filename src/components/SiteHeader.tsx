...
export default function SiteHeader() {
  ...
  return (
    <>
      ...
      {/* 백드롭 — header 바깥에서 독립적인 z-index 확보 */}
      <div
        aria-hidden={!isMenuOpen}
        onClick={() => setIsMenuOpen(false)}
        className={`fixed inset-0 z-[200] bg-black/50 transition-opacity duration-300 sm:hidden ${
          isMenuOpen ? "opacity-100" : "pointer-events-none opacity-0"
        }`}
      />

      {/* 사이드바 — header 바깥에서 독립적인 z-index 확보 */}
      <div
        role="dialog"
        aria-modal="true"
        aria-label="모바일 메뉴"
        className={`fixed right-0 top-0 z-[210] h-full w-72 bg-white shadow-2xl transition-transform duration-300 ease-in-out sm:hidden ${
